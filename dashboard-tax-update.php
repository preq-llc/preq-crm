...
<body>
                        ...
                        <div class="col">
                            ...
                            <div class="h-100">
                                ...
                                <div class="row">
                                            ...
                                            <div class="card-body">
                                                ...
                                                <div class="d-flex align-items-center">
                                                    <div class="flex-grow-1 overflow-hidden">
                                                        <p class="text-uppercase fw-medium text-muted text-truncate mb-0">
                                                                <?php 
                                                                    echo ($logged_in_user_name == 'TAX') ? 'TRANSFER' : 'TRANSFER'; 
                                                                ?>
                                                            </p>
                                    ...
                                    if ($logged_in_user_name != 'PSPM' && $logged_in_user_name != 'TAX') {
                                                    ...
                                                    <table id="export_table" class="table table-borderless table-centered align-middle table-nowrap mb-0">
                                                        <thead class="text-muted table-light tablehead">
                                                            <tr>
                                                                ...
                                                                <?php if ($logged_in_user_name != 'PSPM' && $logged_in_user_name != 'TAX'): ?>
    ...
    <div id="userstatus" class="modal fade" tabindex="-1" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-w75">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="myModalLabel">User Status</h5>
    ...
    <!-- Table Container -->
    <table id="export_table_datewise" class="table table-bordered" style="display:none;">
        <thead>
    ...
    <script>
        $(document).ready(function() {
            ...
            $.ajax({
                url: 'ajax/report/dashboard-beta.php?action=selectcamp',
                type: 'get',
                data: {
            ...
            $('body').on('click', '#getRecord', function() {
                ...
                if (startDate == "") {
                    ...
                    $.ajax({
                        url: 'ajax/report/dashboard-beta.php?action=report',
                        type: 'get',
                        data: {
                            startDate: startDate,
                            endDate: endDate,
                        ...
                        $.ajax({
                            url: "ajax/report/dashboard-beta.php?action=liveagent",
                            type: "GET",
                            data: {
                                startDate: startDate,
                                endDate: endDate,
                            ...
                            if(campaign_name == "TAX")
                            {
                                $.ajax({
                                    url: "ajax/report/dashboard-beta.php?action=gettax_submit",
                                    type: "GET",
                                    data: {
                                        startDate: startDate,
                                        endDate: endDate,
                                    ...
                                    success: function(response) {
                                        ...
                                        var data = JSON.parse(response);
                                        console.log(data.data[0].Totalcalls);
                                        $('#total_billable').html(data.data[0].Totalcalls)
                                    //     if (data.status == 'Ok') {
                                    //         var activeCount = data.data.length;
                                    //         // $('#active_agent')
                                    //         $('#active_agent').attr('data-target', activeCount).html(activeCount);
            ...
            function agentWise() {
                ...
                record.forEach(function(arr, idx) {
                    ...
                    if (current_username == 'PSPM' ) {
                        ...
                        <tr>
                            <td>${camp_name}</td>
                           <td style="cursor:pointer;" data-campaign_id="${camp_name}" data-agent="${arr.users}"  class="user_stats text-primary">${arr.users}</td>
                            <td>${arr.username}</td>
                ...
                if (type == 'agent') {
                    $('.tablehead').html(`
                        <tr>
                            ...
                            <th scope="col">Total Calls</th>
                            <th scope="col">Transfer</th>
                            ${current_username != 'PSPM' && current_username != 'TAX' ? '<th scope="col">Billable</th>' : ''}
                            <th scope="col">TPH</th>
                            ${current_username != 'PSPM' && current_username != 'TAX'? '<th scope="col">BPH</th><th scope="col">SPC</th>' : ''}
                        </tr>
                    `);
                    agentWise();
            ...
            function renderFinalTable(report, campaign_name, startDate, endDate) {
                ...
                dates.forEach(date => {
                    ...
                    $('.table_result2').append(`
                        <tr style="background:#f6f6f6; font-weight:bold;">
                            ...
                            <td>${(dateTotals.Transfer / (dateTotals.Hrs / 3600)).toFixed(2)}</td>
                            <td>${(dateTotals.Totalcalls > 0 ? (dateTotals.Transfer / dateTotals.Totalcalls).toFixed(2) : '0.00')}</td>
                        </tr>
                    `);
                });
                ...
                $('.table_result2').append(`
                    <tr style="background:#ffe6cc; font-weight:bold;">
                        ...
                        <td>${(grandTotal.Transfer / (grandTotal.Hrs / 3600)).toFixed(2)}</td>
                        <td>${(grandTotal.Totalcalls > 0 ? (grandTotal.Transfer / grandTotal.Totalcalls).toFixed(2) : '0.00')}</td>
                    </tr>
