<?php
    ...
    if($action == 'gettax_submit')
    {
        // print_r($slctcampvalueview);
        $ss="select * from campaigns_details where campaign_value='$slctcampvalueview' AND status='ACTIVE'";
        $rr=mysqli_query($conn,$ss)or die(mysqli_error($conn));
        $fr=mysqli_fetch_assoc($rr);
        ...
        if($slctcampvalueview != 'FTE_ALL')
        {
            ...
            try
            {
                ...
                if($fromdatevaluesview != $today_date)
                {
                   $recording_db = 'recording_log';
                }
                else
                {
                   $recording_db = 'recording_log_archive';

                    // recording_log
                }
                $sth = $con->prepare("SELECT count(*) as Totalcalls from recording_log where recording_log.start_time>= '$fromdatevaluesview 00:00:00' AND recording_log.start_time <= '$todatevaluesview 23:59:59' AND recording_log.channel LIKE '%SIP%' AND recording_log.length_in_sec>=83 ORDER BY recording_log.recording_id ASC");
