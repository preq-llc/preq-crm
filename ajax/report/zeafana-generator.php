...
<?php
    ...
    {
        // echo date('Y-m-d');
        $fromdatevaluesview=date('Y-m-d');
        $todatevaluesview=date('Y-m-d');
    ...
    if($action == 'report')
    {
        ...
        mysqli_query($conn, "TRUNCATE TABLE `zeafana_temp_stats`");
        ...
        foreach($getresponse as $response)
        {
            ...
            if($camp != 'EDU_TEST')
            {
                ...
                $sql = mysqli_query($conn, "SELECT * FROM `zeafana_temp_stats` WHERE `campaign_name` = '$camp' AND `agent_id` = $users");
                ...
                {
                    mysqli_query($conn, "INSERT INTO `zeafana_temp_stats`(`campaign_name`, `agent_id`, `total_hrs`, `total_calls`, `transfer`, `billable`) VALUES ('$camp', '$users', '$Hrs', '$Totalcalls', '$successtransfer', '$Transfer')");
        ...
        foreach($gettingAction as $eakAction)
        {
            ...
            else
            {
                $countingPoint = $edu_test_Transfer;
                    ...
                    $getUser = mysqli_query($conn, "SELECT agent_id FROM `zeafana_temp_stats` WHERE `agent_id` LIKE 'ZD%'");
                    $agents = [];
                    ...
                    for ($i = 1; $i <= $countingPoint; $i++) {
                        ...
                        if ($eakAction == 'transfer') {
                            mysqli_query($conn, "UPDATE `zeafana_temp_stats` SET `transfer` = `transfer` + 1 WHERE `agent_id` = '$thisAgentId'");
                        } else {
                            mysqli_query($conn, "UPDATE `zeafana_temp_stats` SET `billable` = `billable` + 1 WHERE `agent_id` = '$thisAgentId'");
                        }
        ...
        $getFinSql = mysqli_query($conn, "SELECT * FROM `zeafana_temp_stats` WHERE 1 $campQuery");
