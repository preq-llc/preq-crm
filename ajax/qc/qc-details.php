...
if($action == 'getqcdetails')
{
    ...
    if($slctcampvalue != 'FTE_ALL')
    {
        ...
        $sth = $conn->prepare("SELECT * FROM `QC_Reports` WHERE `QC_dispo`!='' AND `timestamp`>='$fromdatevalue 00:00:00' AND `timestamp`<='$todatevalue 23:59:59' $phone_number_query $campaign_query $callCenter_query $qcDispo_query");
    }
    else
    {
        $getresponse = [];
        ...
        foreach($campaigns as $camp)
        {
            ...
            $sth = $conn->prepare("SELECT * FROM `QC_Reports` WHERE `QC_dispo`!='' AND `timestamp`>='$fromdatevalue 00:00:00' AND `timestamp`<='$todatevalue 23:59:59' AND camp='$camp' $phone_number_query $callCenter_query $qcDispo_query");
...
else if($action == 'scorecard')
{
    ...
    if($slctcampvalue != 'FTE_ALL')
    {
        $sth = $conn->prepare("SELECT camp,count(id) as total_call,agent_username,sum(case when QC_Reports.QC_dispo = 'QC' then 1 else 0 end) AS QC,count(QC_dispo) as score_card FROM `QC_Reports` WHERE `timestamp`>='$fromdatevalue 00:00:00' AND `timestamp`<='$todatevalue 23:59:59' $campaign_query $callCenter_query  GROUP by agent_username order by camp ASC");
    }
    else
    {
        $getresponse = [];
