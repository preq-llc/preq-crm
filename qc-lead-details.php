...
<head>
    ...
    <meta charset="utf-8" />
    <title>QC Lead Details | Zealous - Dialer CRM</title>
    ...
    <script>
         $(document).ready(function(){
            ...
            console.log(currentUserRole);
              ...
              if(currentUserRole =='qcagent'){

                $("#getRecord").on("click",function(){

                  var fromdate = $("#startDate").val();
                  var todate = $("#endDate").val();
                  var slctcamp=$("#campaign").val();
                  
                  if(slctcamp=='')
                  {
                      alert('Please Choose a Campaign!');
                  }
                  else if(fromdate!='' && todate!='' && slctcamp!='')
                  {
                    fromdatevalue=fromdate;
                    todatevalue=todate;
                 
                    slctcampvalue=slctcamp;

                    overall_post_working(fromdatevalue,todatevalue,slctcampvalue);
                  }

                });

                function overall_post_working(fromdatevalue,todatevalue,slctcampvalue)
                {
                          

                          $.ajax({
                            url: "ajax/qc/fetch_request.php",
                            type: "GET",
                            data: {
                           ...
                           success:function(data){
                            ...
                            if(conv.status == 'HaveOne')
                            {
                              alert('Please Audit Exist File');
                            }
                ...
                function overall_fetch_value()
                {

                    $.ajax({
                    url: "ajax/qc/qc_lead_reports.php",
                    type: "GET",
                    data: {
                    ...
                    $.each( datasucess, function (index,user) {
                        ...
                        if(user.QC_dispo!=null)
                        {
                            qcdisp=user.QC_dispo;
                              bgcolor='green';
                        }
                        else
                        {
                     ...
                     if(user.camp == 'EDU_SB1')
                      {
                              var modifiedUrl = user.audio_link_url;
                      ...
                      else
                      {
                        ...
                        if(user.QC_Audited!=null)
                        {
                            auditedby=user.QC_Audited;

                            qc_url='<td><a href="details_fetch.php?action=showleads&fname='+user.first_name+'&lname='+user.last_name+'&audio='+modifiedUrl+'&user='+user.agent_username+'&time='+user.timestamp+'&leadid='+user.leadid+'&camp='+user.camp+'&phone='+user.phone_number+'" target="_blank"><button class="btn btn-primary" id="bk">'+qcdisp+'</button></a></td>';
                            $("#qc_feedbacks").show();
                        $("#bk").hide();
                        }
                        else
                    ...
                    '<td>'+user.camp+'</td>'+
                    '<td>'+user.leadid+'</td>'+
                    '<td style="display:none">'+modifiedUrl+'</td>'+
                       qc_url+
                    '<td>'+auditedby+'</td>'+
