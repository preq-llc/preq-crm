...
<body>
    ...
    <div id="layout-wrapper">
                                ...
                                <div class="card-header align-items-center d-flex">
                                    ...
                                    <?php 
                                        ...
                                        <div class="flex-shrink-0">
                                            <select name="" id="type_dl" class="form-select form-select-sm">
        ...
        <table id="export_table_download" class="table table-borderless table-hover table-centered align-middle table-nowrap mb-0 d-none">
            <thead class="text-muted table-light">
                <tr>
                    <th scope="col">Sno</th>
                    <th scope="col">Lead ID</th>
         ...
         $('.js-example-basic-multiples').select2();
            ...
            $('body').on('click', '#getRecord', function() {
                ...
                // }
                    ...
                    $.ajax({
                        url: 'ajax/report/leadreport_test.php?action=getleadtotal',
                        type: 'get',
                        data: {
                        ...
                        success: function(response) {
                            ...
                            handleTotalCount(fromdatevalue, todatevalue, slctcampvalue, total_count, call_centervalue,agent_id);
            ...
            function handleTotalCount(fromdatevalue, todatevalue, slctcampvalue, total_count, call_centervalue,agent_id) {

                 current_username = $("#current_username").val();
                $.ajax({
                    url: 'ajax/report/leadreport_test.php?action=getleadreport',
                    type: 'get',
                    data: {
                        fromdatevalue: fromdatevalue,
                        todatevalue: todatevalue,
                    ...
                    success: function(response) {
                        ...
                        record.map(function(arr, idx) {
                            ...
                            if (current_username === 'TAX') {
                                if (arr.status === 'SUBMIT') {
                                    status = 'ST';
                                    statusname = 'SUCCESSFUL TRANSFER';
                                } else {
                                    status = arr.status;
                                    statusname = arr.full_status;
                                }
                            } else {
                                status = arr.status;
                                statusname = arr.full_status;
                                            ...
                                            <div class="card-body" style="z-index:1 ;">
                                                <div class="d-flex align-items-center">
                                                    <div class="flex-grow-1 overflow-hidden">
                                                        <p class="text-uppercase fw-medium text-muted text-truncate mb-3">${status}</p>
                                                ...
                                                <p class="text-secondary">${statusname}</p>
                                            </div><!-- end card body -->
                                        </div><!-- end card -->
                                    </div>
                                `);
            ...
            $('body').on('click', '.dispoStatsCard', function() {
                ...
                $.ajax({
                    url: 'ajax/report/leadreport_test.php?action=getdisporeport',
                    type: 'get',
                    data: {
                    ...
                    success: function(response) {
                        ...
                        record.map(function(arr, idx) {
                            ...
                            if (currentUserRole == "teamleader" || currentUserRole == "admin" || currentUserRole == "superadmin" || currentUserRole == "qcadmin" || currentUserRole == "support" || currentUserRole === "clientadmin") {
                                ...
                                else if(current_username === 'TAX'){


                                    maskedNumber = arr.filename.slice(-10);
                            ...
                            $('.table_result').append(`
                                    <tr>
                                        ...
                                        <td style="cursor:pointer;" data-datetime="${arr.event_time}" data-agent="${arr.agent}" data-leadid="${arr.lead_id}" data-campaign_id="${arr.campaign_id}" data-phoneno="${arr.filename.slice(-10)}"  data-dispo="${dispView}" class="dispoBtn text-primary">${dispView}</td>
                                    </tr>
                                `);
            ...
            $('body').on('click', '.dispoBtn', function() {
                ...
                $('#myModal').modal('show');
                $.ajax({
                    url: 'ajax/report/leadreport_test.php?action=getDispoDetails',
                    type: 'get',
                    data: {
                        leadID: leadID,
                    ...
                    success: function(response) {
                        ...
                        if (
                            ...
                            maskedNumber = record.phone_number;
            ...
            //     $.ajax({
            //             url: 'ajax/report/leadreport_test.php?action=getDispoAudio',
            //             type: 'get',
            //             data: {
            //                 leadID:leadID,
            //                 campaign_id:campaign_id,
            ...
            //     $.ajax({
            //         url: 'ajax/report/leadreport_test.php?action=getRecordingStatus',
            //         type: 'get',
            //         data: {
            //             leadID: leadID,
            //             campaign_id: campaign_id,
        ...
        success: function(response) {
            ...
            if (response.data && response.data.length > 0) {
                ...
                response.data.forEach(function(item) {
                    var record = item.location;
                    var recordingpath = item.recordingpath;
                    var showrecord = item.showrecord;
                    ...
                    // Access control
                    if (
                        currentUserLoggedIN != 'SHOW' &&
                        currentUserLoggedIN != 'PSPM' &&
                        (showrecord == 1 || 
                        currentUserRole == 'superadmin' || 
                        currentUserRole == 'teamleader' || 
                        currentUserLoggedIN == 'ZD15' || 
                        currentUserRole == 'clientadmin')
                    ) {
                        html += `
                            <audio class="getthisaudio" preload="metadata" controls style="margin-bottom:10px; display:block;">
                                <source src="${audio}" type="audio/mpeg">
                            </audio>
                        `;
                    }
                ...
                // Force reload metadata for all audios
                $(".getthisaudio").each(function () {
                    this.load();
                });
            } else {
                $('.audioRec').html('<p>No recordings found.</p>');
            }
        }
    });
}
            ...
            $('#export_excel').on('click', function() {
               ...
               var all_dl_report = [];
                    ...
                    Promise.all(promises).then(function() {
                        var table = document.getElementById('export_table_download');
            ...
            if(currentUserRole =='clientadmin'){
                ...
                $('#myModaltwo').modal('show');
                        ...
                        $('body').on('click', '.updatedispo', function() {
                            ...
                            $.ajax({
                                                url: 'ajax/report/leadreport_test.php?action=updateDispoDetails',
                                                type: 'get',
                                                data: {
                                                    leadID: leadID,
                                                    campaign_id: campaign_id,
            ...
            $('body').on('change', '#campaign', function () {
                ...
                $.ajax({
                    url: "ajax/report/leadreport_test.php?action=liveagent",
                    type: "GET",
                    data: {
                        startDate: startDate,
                        endDate: endDate,
                            ...
                            if (data.status === 'Ok') {
                                ...
                                record.forEach(function (arr) {
                                 ...
                                 if(agent_id != null){
                                    ...
                                    console.log('aruntest',agent_id);


                                        $('#getRecord').trigger('click');
