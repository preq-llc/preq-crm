...
<head>
    ...
    <!-- App Css-->
    <link href="assets/css/app.min.css?sdf3535435" rel="stylesheet" type="text/css" />
    <!-- custom Css-->
    <link href="assets/css/custom.min.css?345345sdfsdf" rel="stylesheet" type="text/css" />
...
<body>
    ...
    <div class="offcanvas offcanvas-end" id="campControlCanvas">
        ...
        <div class="offcanvas-body">
            <div class="row campcanvasbody">
                ...
                <div class="col-12 pt-3">
                    <label for="">Status</label>
                    <select name="" id="statuscampaign" class="col-12 form-control">
                        <option value="ACTIVE" selected>Active</option>
                        <option value="In-Active">Deactive</option>
                    </select>
                </div>
                <div class="col-12 pt-3">
                    <label for="">Recording Path</label>
                    <input type="text" class="form-control" value="../Preq-zenba/rec" name="" id="recordingpath">
                </div>
                <div class="col-12 pt-3">
                    <label for="">Show Recordings</label>
                    <input type="text" class="form-control" name="" id="show_recordings">
                </div>
                <!-- <div class="col-12 pt-3">
                    <label for="">Status</label>
                    <select name="" id="status" class="form-select">
    ...
    <!-- App js -->
    <script src="assets/js/app.js?34534534534554"></script>
           ...
           // var tagify = new Tagify(input);
            ...
            $('body').on('click', '#updateUser', function() {
                ...
                if (username == "") {
                    ...
                    $.ajax({
                        url: 'ajax/admin/adminPanel_test.php?action=userupdate',
                        type: 'get',
                        data: {
                            username: username,
                            password: password,
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (data.status == 'Ok') {
                            ...
                            $('#statuscampaign').val(record[0].status);
                            $('#recordingpath').val(record[0].recordingpath);
                            $('#show_recordings').val(record[0].show_recordings);
            ...
            $('body').on('click', '#updatcamp', function () {
                ...
                //const getVal = id => $(`#${id}`).length ? $(`#${id}`).val().trim() : '';

                const camp_ip        = getVal('camp_ip');
                const camp_name      = getVal('camp_name'); // ✅ fixed
                const zenba_no       = getVal('zenba_no');
                const zenba_ip       = getVal('zenba_ip');
                const zenba_db       = getVal('zenba_db');
                const zenba_username = getVal('zenba_username');
                const zenba_pass     = getVal('zenba_pass');
                const camp_db        = getVal('camp_db');
                const camp_user      = getVal('camp_user');
                const camp_pass      = getVal('camp_pass');
                const statuscampaign        = getVal('statuscampaign');
                alert(statuscampaign)
                const recordingpath   = getVal('recordingpath');
                const show_recordings = getVal('show_recordings');

                const campaction = $(this).data('campaction') || 'create';

                // ✅ Validation checks
                if (!camp_name) return toastr.warning('Enter Campaign Name');
                if (!camp_ip) return toastr.warning('Enter Campaign IP');
                if (!camp_db) return toastr.warning('Enter Campaign DB Name');
                if (!camp_user) return toastr.warning('Enter Campaign DB Username');
                if (!camp_pass) return toastr.warning('Enter Campaign DB Password');
                if (!recordingpath) return toastr.warning('Enter recordingpath');
                if (!show_recordings) return toastr.warning('Enter show_recordings');
                // ✅ Safe check to make sure data is valid
                const dataToSend = {
                    camp_ip,
                    camp_name,
                    zenba_no,
                ...
                $.ajax({
                    url: 'ajax/admin/adminPanel_test.php?action=createCamp',
                    type: 'GET',
                    data: dataToSend,
                    success: function (response) {
                        try {
                            ...
                            if (data.status === 'Ok') {
                                const msg = (campaction === 'create') ? 'New Campaign Created!' : 'Campaign Updated!';
                                toastr.success(msg);
                                 location.reload();
                            } else if (data.status === 'Exists') {
                                toastr.warning('Campaign name already exists!');
                            } else {
                            ...
                            toastr.error("Error in server response.");
                        }
                    },
                    error: function () {
